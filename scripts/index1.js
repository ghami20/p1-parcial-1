...
document.getElementById('form-disco').addEventListener('submit', function (event) {
  ...
  const id = form['editando-id'].value || form.codigo_disco.value.trim();
  const nombre = form.nombre.value.trim();
  const artista = form.artista.value.trim();
  const portada = form.portada.value.trim();

  if (!nombre || !artista || !portada || !id) {
  ...
  if (pistas.length === 0) {
    alert("Tenes que agregar al menos una pista.");
    return;
  }

  const discos = JSON.parse(localStorage.getItem('discos')) || [];
  if (discos.some(d => d.id === Number(id))) {
    alert("Ya cargaste un disco con este ID. Intenta con otro ID.");
    return;
  }
